...
function stripKindPrefix(raw: string): string {
  ...
  for (const prefix of ["user:", "channel:", "group:", "dm:", "chat:"]) {
    if (lower.startsWith(prefix)) {
...
function inferPeerKind(params: {
  ...
  if (params.resolvedTarget?.kind === "user") {
    return "direct";
  }
  if (params.resolvedTarget?.kind === "channel") {
    return "channel";
  }
  if (params.resolvedTarget?.kind === "group") {
    return "group";
  }
  return "direct";
}
