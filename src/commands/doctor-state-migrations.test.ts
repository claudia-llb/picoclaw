...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    writeJson5(path.join(legacySessionsDir, "sessions.json"), {
      ...
      "telegram:channel:C123": { sessionId: "c", updatedAt: 30 },
    ...
    expect(store["agent:main:telegram:channel:c123"]?.sessionId).toBe("c");
  ...
  it("migrates legacy Telegram auth files without touching oauth.json", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {};
    ...
    const target = path.join(oauthDir, "telegram", "default");
  ...
  it("lowercases agent session keys during canonicalization", async () => {
    ...
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), {
      "agent:main:telegram:channel:C123": { sessionId: "legacy", updatedAt: 10 },
    });
    ...
    expect(store["agent:main:telegram:channel:c123"]?.sessionId).toBe("legacy");
    expect(store["agent:main:telegram:channel:C123"]).toBeUndefined();
  });
