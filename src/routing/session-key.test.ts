...
describe("session key backward compatibility", () => {
  it("classifies legacy :dm: session keys as valid agent keys", () => {
    ...
    expect(classifySessionKeyShape("agent:main:telegram:dm:+15551234567")).toBe("agent");
    expect(classifySessionKeyShape("agent:main:telegram:dm:user123")).toBe("agent");
  });
  ...
  it("classifies new :direct: session keys as valid agent keys", () => {
    ...
    expect(classifySessionKeyShape("agent:main:telegram:direct:+15551234567")).toBe("agent");
    expect(classifySessionKeyShape("agent:main:telegram:direct:user123")).toBe("agent");
  });
});
